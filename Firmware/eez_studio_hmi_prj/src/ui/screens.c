...
static void event_handler_cb_schermata_jogger_gradi_per_click_ralla(lv_event_t *e) {
    ...
    if (event == LV_EVENT_VALUE_CHANGED) {
        ...
        if (tick_value_change_obj != ta) {
            const char *value = lv_textarea_get_text(ta);
            assignStringProperty(flowState, 34, 3, value, "Failed to assign Text in Textarea widget");
        }
    }
    if (event == LV_EVENT_PRESSED) {
        e->user_data = (void *)0;
...
void tick_screen_schermata_jogger() {
    ...
    {
        const char *new_val = evalTextProperty(flowState, 34, 3, "Failed to evaluate Text in Textarea widget");
