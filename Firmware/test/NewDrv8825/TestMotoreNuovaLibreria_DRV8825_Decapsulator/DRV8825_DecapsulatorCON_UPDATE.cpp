...
#include "DRV8825_DecapsulatorCON_UPDATE.hpp"
#include "esp_timer.h"
...
drv_err_t DRV8825::begin(uint8_t DIR, uint8_t STEP, uint8_t EN, uint8_t RST, uint8_t SLP, uint16_t number_of_steps_per_revolution)
{
  ...
  esp_err_t esp_err = rmt_new_tx_channel(&rmt_tx_cfg, &this->_rmtChannel);
  if(esp_err != ESP_OK)
  {
...
void DRV8825::setRMT(uint64_t period_us)
{
  ...
  Serial.printf("_stepPulse[0].duration1 : %d\n", this->_stepPulse[0].duration1);
  Serial.printf("period_us : %d\n", period_us);
  Serial.printf("DRV8825_RMT_PULSE_US : %.1f\n", DRV8825_RMT_PULSE_US);
  Serial.printf("period_us : %.2f\n", (double)period_us);
  Serial.printf("(double)period_us / DRV8825_RMT_PULSE_US : %.2f\n\n\n", (double)period_us / DRV8825_RMT_PULSE_US);
