...
void init_style_arancione_MAIN_CHECKED(lv_style_t *style) {
    lv_style_set_bg_color(style, lv_color_hex(0xf39d21));
    lv_style_set_text_color(style, lv_color_hex(0x000000));
};
...
lv_style_t *get_style_arancione_MAIN_CHECKED() {
    ...
    if (!style) {
        style = (lv_style_t *)lv_malloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_arancione_MAIN_CHECKED(style);
    }
    return style;
};
...
void add_style_arancione(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_arancione_MAIN_CHECKED(), LV_PART_MAIN | LV_STATE_CHECKED);
};

void remove_style_arancione(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_arancione_MAIN_CHECKED(), LV_PART_MAIN | LV_STATE_CHECKED);
};
