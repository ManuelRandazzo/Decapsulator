 ...
 */
inline void INTERRUPTS::resumeISR()
{
  if(!__isInterruptAttached)
  {
    TaskIT.Resume();
 ...
 */
inline void INTERRUPTS::suspendISR()
{
  if(__isInterruptAttached)
  {
    TaskIT.Suspend();
 ...
 */
inline void INTERRUPTS::changePinIT(uint8_t pin, uint8_t modePin, uint8_t IT_TriggerMode)
{
 ...
 */
inline TaskHandle_t INTERRUPTS::getHandlerIT()
{
  return TaskIT.getHandler();
}

/// Metodo che cambia dinamicamente la funzione gestita nell'ISR
inline void INTERRUPTS::setISR(void (*UserISR)())
{
...
/// Metodo che cambia dinamicamente il metodo gestito nell'ISR
template <class refClass>
inline void INTERRUPTS::setISR(refClass* thisPointer, void (refClass::*methodToExecute)())
{
